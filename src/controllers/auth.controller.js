...
export async function login(req, res, next) {
  try {
    ...
    const jwt_token = signToken({userId: user._id, email: user.email, role: user.role})

    const userObj = user.toObject()
    delete userObj.password

    return res.status(200).json({token: jwt_token, user : userObj})
  } catch (error) {
    next(error);
  }
}
...
export async function me(req, res, next) {
  try {
    ...
    return res.status(401).json({error : { message : "User does not exists"}})
  }
}
